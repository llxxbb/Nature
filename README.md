...
Nature 用 `Relation`来表示关系，用于连接两个`Meta`，因此它是一对一的。比如“订单”和“生产申请单”之间的关系。`Relation` 的这种形式有下面几个重要的好处：
...
相对于传统开发模式，目标的可视化本身就是一件很困难的事，要想做到全面实时几乎是不太可能。而Nature 中的 `Relation` 将功能性的东西隐藏了起来，只把目标性的内容展示给决策人员。Nature通过`Relation`将所有的目标串接成一个庞大的业务网，这**是整个业务系统的一个完整的实时的结构快照**。这使得**目标之间的推理变得清晰明了**，也就解决了目标覆盖的问题。同时也避免使业务设计人员陷入功能的漩涡。就像一个地球仪一样，所有的东西都在那里，但里面没有飞机、轮船和火车等功能性的东西，让你聚焦你想要的东西。

**目标驱动可以给你一个清晰的，简单的，更层次的视角去审视业务布局的合理性，而功能驱动会让你感觉到混乱、困惑。**
...
Nature 和 `Converter` 协作的方式实际上可以看做是 FaaS(Function as a Service) 或者 Serveless 的一种形式，但我不想使用这个概念，因为`FaaS` 是面向功能的，而Nature 是面向业务目标的。

### 数据库

Nature 中的 `Relation` 是借鉴了关系数据库的设计思路，用统一的一对一这种模型来表达一对多，多对多、多对一等复杂业务关系，使得业务的关系模型管理变得简单。同时 Nature 提供了对业务数据的存储，从某种意义上讲也是一款高级别的数据库产品。但Nature 并不是躲在代码背后的一个技术产品，而是引导业务逻辑的业务作战指挥室。
