...
系统复杂的一个重要原因在于**技术团队主导数据**，因为这会导致**数据与技术的耦合**！最直接的证据就是接口，接口是一种技术实现，但作为数据的入参和出参必须依附于接口才有意义。另一个证据就是，很多时候只有深入代码我们才能了解数据之间的关系变化。
...
- **全局视角**：能够提供简明的全局数据视图，为业务决策提供更好的支持，利于决策的传达并提升全员的参与度。
- **数据共享**：数据是企业的重要诉求，数据是不同部门间协作的桥梁，所以数据应该是共享的而不是技术团队的的私有物品。非技术团队也应该有管理数据的能力。
- **过程可见**：使数据流转的所有环节可见且易于调控。而不是去面对晦涩难懂的接口调用。避免陷入功能的泥潭而迷失目标，且对技术团队进行强制约束。
...
Nature的数据定义（称之为 `Meta`）可看作是**大众化的目标定义**。之所以称之为目标定义，是因为系统的产物就是数据，所以 Nature 的数据定义定义了系统的目标；之所以称之为大众化的，是因为不需要很高的技术门槛。不同于数据库的数据定义：
...
- 数据处理方式的统一，可以让 Nature 进一步赋能，如数据版本化，幂等、环境异常重试、延迟执行、数据不变性、数据一致性等，这进一步减轻了技术团队的负担。
...
自然的另外一层意思是**自然选择**，自然选择是生态系统的基本法则，法则没有去控制，无为而治，但却显现出了强大的无形控制。对于一个复杂的业务系统来讲，与其主动去控制所有的对象，不如大家都遵循一个法则更有效。 
