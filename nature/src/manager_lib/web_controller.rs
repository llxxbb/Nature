...
/// add one meta
#[post("/meta/add")]
async fn meta_add(path: web::Path<RawMeta>) -> HttpResponse {
    let rtn = D_M.insert(&path.into_inner()).await;
    web_result(rtn)
}

#[get("/meta/delete/{name}")]
async fn meta_delete(path: web::Path<String>) -> HttpResponse {
    let meta = Meta::from_string(&path.into_inner());
    if meta.is_err() {
...
#[get("/metaUsed/{name}")]
async fn meta_used(path: web::Path<String>) -> HttpResponse {
    // TODO
    HttpResponse::Ok().body(format!("get from: {}", path.into_inner()))
}
