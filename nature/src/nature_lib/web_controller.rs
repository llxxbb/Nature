...
#[post("/input")]
async fn input(instance: Json<Instance>, context: Data<WebContext>) -> HttpResponse {
    let x = IncomeController::input(instance.0, context).await;
    web_result(x)
}

/// **Note** This do not receive System `Meta`'s instances
#[post("/inputJS")]
async fn input_js(instance: Json<InstanceVO>, context: Data<WebContext>) -> HttpResponse {
    let para = instance.0.try_into();
    if para.is_err() { return web_result(para); }
    let x = IncomeController::input(para.unwrap(), context).await;
...
#[post("/self_route")]
async fn self_route(instance: Json<SelfRouteInsVO>, context: Data<WebContext>) -> HttpResponse {
    let para = instance.0.try_into();
    if para.is_err() { return web_result(para); }
    let x = IncomeController::self_route(para.unwrap(), context).await;
    web_result(x)
}

#[post("/callback")]
async fn callback(delayed: Json<DelayedInstances>, context: Data<WebContext>) -> HttpResponse {
    let x = IncomeController::callback(delayed.0, context).await;
    web_result(x)
}

#[post("/batch")]
async fn batch(parallel_batch: Json<Vec<Instance>>, context: Data<WebContext>) -> HttpResponse {
    let x = IncomeController::batch(parallel_batch.0, context).await;
    web_result(x)
}

#[post("/batchJS")]
async fn batch_js(parallel_batch: Json<Vec<InstanceVO>>, context: Data<WebContext>) -> HttpResponse {
    ...
    let x = IncomeController::batch(para.into_iter().map(|o| { o.unwrap() }).collect(), context).await;
    web_result(x)
}

#[post("/task/redo")]
async fn redo_task(task: Json<RawTask>, context: Data<WebContext>) -> HttpResponse {
    let x = IncomeController::redo_task(task.0, context).await;
    web_result(x)
}
