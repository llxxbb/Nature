...
其中之一是**决策的标准化**，传统方式下的决策是十分笨重的，必须和执行一起进行通盘考虑，然后经由部署的系统来呈现，形式各异，过程缓慢；为什么这么说呢？因为代码里有思想，虽然这有点违反直觉，但“最好的文档就是代码”已经说明了问题。而借助于 Nature 我们只需要按照统一的形式简单配置一下便可产生实质性的效力。然而 Nature 并不局限于形式的简化，它更大的意义在于**提炼决策的内含**，因为相对于传统方式 Nature 可以大幅度减少执行层面的考虑，这会减少了大量的不必要的中间决策产物，大幅度缩短决策过程，并提升决策质量。
...
总结一下，传统系统是决策和执行的统一体，不可分割。分散的系统导致了分散的决策，很难形成合力来有效**驾驭自身**。
