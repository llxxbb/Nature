...
Nature 并不是《易经》的系统化，《易经》虽然形式上简单，但要想运用自如绝非等闲之辈所能及，因此请放心，Nature 不是一个玄学系统。这里只用于 Nature 的设计理念，其它文档都不会引用《易经》的任何内容。本系统之所以命名为 Nature 也有摒弃玄学之意，同时也期望 Nature 像科学规律一样能够广泛适用于各种复杂现象。

### 两仪

我们先用《易经》中的两仪来解决第一个复杂度，也是最为核心的一个复杂度：**业务系统的划分**。

传统的业务系统大都是按业务的**垂直领域**进行划分的。拿电商来举例，系统大体上可以分为交易、库房、配送、售后、财务、用户等等，这样的划分大多与组织机构的划分保持映射关系，具有很好的直观性，这在历史发展进程中起到重要作用，也是业界的**普遍常识**。但随着系统规模的不断庞大，会暴露出**决策固化**的问题。

系统是设计的产物，而设计可以看做是我们的决策，因为它包含了我们的目的以及为了完成目的所采用的方法规则等；同时系统也是执行的载体，一旦部署据忠实的履行职责。也就是说**决策和执行是系统性的一体两面**。随着系统数量的不断膨胀，我们的决策被不断**分散**并被被固化，所以当决策改变时，我们的协作成本便会因系统的关联而成倍增加，尤其是核心链路上的改变将变得**异常艰难**。

这是因为**决策具有联动性**，当我们对其中的一个系统进行改变时，其它协作系统的决策也需要调整，但苦于决策和执行的一体性，不可避免的或多或少的**受现有执行单元的约束**，这表现为相关系统大多数情况下**不能同步上线**。因此我们要考虑系统平滑过渡和兼容性问题，这就需要大量的**额外的决策**并形成了**难以去除的阶段性代码**（程序员很善于做加法而不善于做减法），这无疑会大大增加系统的复杂性。经过长期的迭代后系统就会变得臃肿和难以维护，所以大型业务系统动辄几千人的开发团队就不足为奇了。

虽然我们可以通过良好的设计来规避一些问题，但对于初期快速发展的业务来讲**很难预判哪个是好的设计**，**稍纵即逝的商机**也很难让我们一开始就有一个完美的设计，所以这不太现实。要想从根本上解决这个问题，需要对问题有本质的认知，不难发现造成**问题的根源是：分散的多体决策和一体化的决策与执行**。如果决策和执行能够分开，使得决策链能够有机融合在一起独立变化，就不会出现新旧打架的问题，也就不会出现过渡产物以及日后的后遗症。

分开的决策与执行，需要一个独立的第三方来协调两者，使其发挥作用，Nature 就是这个第三者。这样 Nature 在所有的业务系统上横切了一刀，并形成决策和执行两个集合。如果 Nature 是太极的话，那么这两个集合便是两仪。
...
其中之一是**决策的标准化**，传统方式下的决策必须经由部署的系统来体现才有效力，形式各异，过程缓慢而笨重，而借助于 Nature 只需要按照统一的形式简单的配置一下便可产生实质性的效力。但Nature 并不局限于形式的简化，更大的意义在于提炼决策的内含，因为相对于传统方式 Nature 可以大幅度减少执行层面的考虑，也无意中减少了大量的不必要的中间决策产物，这无疑会大幅度缩短决策过程，并提升决策质量。

其二就是**执行标准化**，因为业务间的耦合被斩断，各个业务处理单元就非常的独立，这既简化了开发，又为并行开发创造了有力条件。另外执行单元的协作都通过 Nature 来调度，这就必须要求执行单元的接口形式统一化、标准化。这样原先系统间需要关注的通讯超时、数据一致性等问题都可以由 Nature 的调度来承担，这进一步简化了开发的工作量。

执行标准化的另一个好处是**数据规范和纯化**。传统数据库里的数据是临时数据，控制数据、技术数据、业务数据等的大杂烩。
