...
Nature 的选择体现在多个方面，如对上游的状态和上下文做出选择。这里举一个支付和发货的例子：当支付状态为“已支付”则生成出库申请单，如果没有支付完成则不动作。很显然我们会使用`if`控制语句，但这是控制！然而使用 Nature 则没有`if` ，你只需要定义一个“支付状态=已支付->出库申请单”的 `Relation`就可以了。说的再简单点，如果你用 `if`  你必须事先做好分配工作，但 Nature 则主张只要我有的都给你，怎么玩你自己选，我不负责分配。

我们再举一个上下文的例子:订单开发票。我们可以建立一个这样的`Relation` ：订单.context.发票=true -> 发票申请，同样的，这个任务不用开发人员进行流程控制。
