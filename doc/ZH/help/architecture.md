...
Nature 是为了简化业务问题而提出的，只有将业务问题用简单的模型来描述，Nature 才能够承载复杂的业务世界。如果我们将业务系统（这里专指计算机系统）看做是一个整体，这个整体就可以看做是道家的太极，而太极生两仪，两仪生四象，然后统领大千世界。Nature 就是按这个思路**自上而下**来统领整个业务系统。

道法自然，所以本系统被命名为 Nature。

### 两仪

对于业务来讲，个人的理解两仪应该是**决策和执行**。下面基于这个论调展开描述。

从宏观上来讲，传统的业务系统大都是按业务的垂直领域进行划分的。拿电商来举例，系统是按领域直接划分的，大体上可以分为交易、库房、配送、售后、财务、用户等等。这样的划分已经远远超出了两仪的边界，而决策和执行则降格为微观的特定系统上，这样在做业务调整时，这个系统的决策不可避免的与其它系统的执行产生纠葛，就要考虑系统平滑过渡和兼容性问题，就会形成难以去除的阶段性代码，就会造成系统的臃肿和难以维护。所以大的电商平台都有几千的开发人员来支撑这些系统。虽然可以通过良好的设计可以规避一些问题，但对于初期快速发展的业务来讲很难预测哪个是好的设计，稍纵即逝的商机也很难让我们一开始就有一个完美的设计。
