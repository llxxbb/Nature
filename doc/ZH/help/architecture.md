...
假设我们要统计一下一个火爆的电商网站的单品销售 top, 每次统计可能涉及到千万数据，传统的基于 sql 的统计已经不太现实。对于这个问题 Nature 提供了一套自己的解决方案。Nature 提供了一个专有的 `MetaType::Loop` ，Loop 可以**驱动**一次处理一批数据，并把未完成的数据放到 Loop 内部的 `Instance` 里，然后把这个未完成的数据带到下一个批次里处理直到完成为止，并输出到设置的 Meta 对应的 Instance里。批量的控制则来源于 Nature 的一个[内置执行器](built-in.md)：`instance-loader` 后面有这样的示例，请参考：[示例及功能讲解](https://github.com/llxxbb/Nature-Demo)。
