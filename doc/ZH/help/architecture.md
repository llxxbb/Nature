...
当我们要对其中的一个系统进行改变时，不可避免的或多或少的与其它系统的现有执行单元产生纠葛，这就要考虑系统平滑过渡和兼容性问题，就会形成难以去除的阶段性代码，就会造成系统的臃肿和难以维护。所以大的电商平台都有几千的开发人员来支撑这些系统。虽然我们可以通过良好的设计来规避一些问题，但对于初期快速发展的业务来讲很难预测哪个是好的设计，稍纵即逝的商机也很难让我们一开始就有一个完美的设计。

问题很明显，如果能够将决策和执行彼此分离，决策的迭代不影响现有的执行，问题就可以解决了。也就是说我们需要将现有的业务系统的划分全部打散，并重新构建成两大系统：决策系统和执行系统。这就是两仪。Nature 用 `Meta` 和 `Relation` 来行驶决策职能，用 `Executor` 来行驶执行职能，决策和执行的
