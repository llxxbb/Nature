...
Nature 是为了**简化现有业务系统**而提出的，既要使用简单又能承载复杂的业务其实是蛮难的，还好中国古人的智慧已经有现成的遗产可以借鉴。《易传》有云：“易有太极，是生两仪，两仪生四象，四象生八卦”，这是诠释《易经》的，《易经》的博大精深 Nature 自然无法体现的，但其简单、朴素的模型对 Nature 的架构有着直接的指导意义。因为它正好满足我们的核心诉求：**用简单的模型表达复杂的事物**。Nature 并不是《易经》的系统化，这里只是借用一下《易经》的哲理来指导 Nature 的设计，同时也期望 Nature 像科学规律一样能够广泛适用于各种复杂现象。

### 决策与执行

我们先来用 Nature 来解决第一个复杂度，也是最为核心的一个复杂度：**业务系统的划分**。

传统的业务系统大都是按业务的**垂直领域**进行划分的。拿电商来举例，系统大体上可以分为交易、库房、配送、售后、财务、用户等等，这样的划分大多与组织机构的划分保持**映射关系**，具有很好的直观性，这在历史发展进程中起到重要作用，这也是业界的**普遍常识**。但随着系统规模的不断庞大，**决策固化**的问题会凸显出来。

系统是决策的载体，因为它包含了我们的目的以及为了完成目的所采用的方法规则等，同时系统也是执行的载体，一旦部署据忠实的履行职责，也就是说决策和执行是系统性的一体两面。随着系统数量的不断膨胀，我们的决策被不断分散并被代码固化。当决策改变时，由于决策的**联动性**，关联系统越多协作成本越高，尤其是核心链路上的改变会变得**异常艰难**。

这是因为决策和执行的一体性造成的，决策的改变不可避免的或多或少的受现有执行单元的约束，这表现为相关系统大多数情况下**不能同步上线**。因此我们要考虑系统**平滑过渡**和**兼容性**问题，这就需要大量的额外的过渡性决策进行支持，而这些决策又形成了**难以去除的阶段性代码**（程序员很**善于做加法**而不善于做减法），这无疑会大大增加系统的复杂性。经过长期的迭代后系统就会变得**臃肿**和**难以维护**，所以大型业务系统动辄几千人的开发团队就不足为奇了。

虽然我们可以通过良好的设计来规避一些问题，但对于初期快速发展的业务来讲很难**预判**哪个是好的设计，**稍纵即逝的商机**也很难让我们一开始就有一个完美的设计，所以这不太现实。我们需要从根本上来解决这个问题，不难发现造成问题的根源是：**分散的多体决策**和**一体化的决策与执行**。如果决策和执行能够分开，使得决策链能够有机融合在一起独立变化，就不会出现新旧打架的问题，也就不会出现过渡产物以及日后的后遗症。

分开的决策与执行，需要一个独立的第三方来协调两者以使其发挥作用，Nature 就是这个第三者。这样 Nature 在所有的业务系统上横切了一刀，并形成决策和执行两个独立的集合。如果 Nature 是太极的话，那么这两个集合便是两仪。

Nature 用 `Meta` 和 `Relation` 来行使决策职能，用 `Executor` 来行使执行职能，这是一种抽象、简洁、规范和统一的形式，所以可以承载各种业务诉求。Nature 通过这种方式**将业务系统间的直接耦合变成了间接耦合**，这将为业务系统的建设带来深远的影响，它不但解决了决策固化的问题，还带来了其它意想不到的好处。

其中之一是**决策的标准化**，传统方式下的决策必须经由部署的系统来体现才有效力，形式各异，过程缓慢而笨重，而借助于 Nature 只需要按照统一的形式简单的配置一下便可产生实质性的效力。但Nature 并不局限于形式的简化，更大的意义在于**提炼决策的内含**，因为相对于传统方式 Nature 可以大幅度减少执行层面的考虑，也无意中减少了大量的不必要的中间决策产物，这无疑会大幅度缩短决策过程，并提升决策质量。
...
执行标准化的另一个好处是**数据规范和纯化**。一般大规模的业务系统，可能有数十上百个数据库上百成千的数据表，这些数据表是临时数据、业务定义，业务控制、技术数据、业务数据等的**大杂烩**。但 Nature 只有三个数据表与业务相关：`Meta` 用于业务定义， `Relation` 用于业务控制，`Instance`用于业务数据。我想这会很大程度上减少不必要的数据存储，减少数据的冗余。





同时解决了现有决策的另一个问题，那就是决策太重，因为一个决策必须要落到代码上才算一个可行的决策。
...
其实业界已经早已经意识到了这一问题，并提出了解决方案：ERP。ERP 曾经是一个非常火的一个概念并衍生了很多商业性的产品，但最近几乎没人再提 ERP了。并不是这一概念不好，而是只是停留在资源层面

虽然业界广泛引入了 ERP 来解决此类问题的复杂度，但始终没有统一的标准

不规范、不彻底，，决策是没有规范和统一的，只能解决局部问题。因此 ERP 本身的复杂并不低，他要对接各类现有的业务系统，而比不是取代现有的业务系统。只是大公司可以玩得起的工具。

在 Nature 里决策和执行时两个抽象的概念，
...
面向功能的业务系统是**面向对象**的基本表现形式，但有人已经对此进行了深刻的批判，请参考《[面向对象编程—价值万亿美元的灾难](https://www.toutiao.com/i6788266897667260931/?tt_from=weixin&utm_campaign=client_share&wxshare_count=1&timestamp=1595718883&app=news_article&utm_source=weixin&utm_medium=toutiao_android&use_new_style=1&req_id=202007260714430100150420261F6A5C4D&group_id=6788266897667260931)》，Nature 则给出了自己的反对意见：

这样的划分会要求每个业务系统都有一套属于自己的完整的独立的**决策和执行机制**，决策和执行是无法分割的，因它们是一套代码的一体两面。这是面向对象编程里的常规产物：将数据和功能封装到一起；数据是目标既决策，方法既执行。这种耦合会对我们的系统迭代造成很大的困难。
